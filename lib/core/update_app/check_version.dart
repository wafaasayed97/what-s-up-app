...
import 'package:what_s_up_app/core/cache/preferences_storage/preferences_storage_keys.dart';
...
class AuthNavigationService implements INavigationService {
  ...
  Future<void> navigate() async {
    ...
    final rememberMe = sl<PreferencesStorage>().getBoolean(
      key: PreferencesKeys.rememberMe,
      defaultValue: true,
    );
    safePrint("remember me ==> $rememberMe");
    safePrint("token  ==> $token");
    if (token == null || token.isEmpty) {
      safePrint("log out");
      // if (UserHelpers.isFirstTime()) {
      //   safePrint("first time");
      //   context.pushReplacement(Routes.onBoardingScreen);
      // } else {
      //   safePrint("not first time");
    ...
    if (localAuthEnabled == 'true') {
      ...
      while (!didAuthenticate && retryCount < maxRetries) {
        ...
        // if (!didAuthenticate) {
        //   retryCount++;
        //   final remaining = maxRetries - retryCount;
        //   if (remaining > 0) {
        //     showError(
        //       S().auth_failed_attempts_left(remaining),
        //       canDismiss: true,
        //     );
        //     ScaffoldMessenger.of(context).showSnackBar(
        //       SnackBar(
        //         content: Text(
        //           S.of(context).auth_failed_attempts_left(remaining),
        //         ),
